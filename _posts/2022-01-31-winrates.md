...
(ABD), so I’d like to think I know what I’m talking about on the
...
data: for *i* ∈ {1, …, *n*}
$$x\_i\\mid p\\stackrel{ind}{\\sim}\\textsf{Bernoulli}(p).$$
...
[model](https://en.wikipedia.org/wiki/Bernoulli_distribution) for the
data: for *i* ∈ {1, …, *n*}
$$x\_i\\mid p\_i\\stackrel{ind}{\\sim}\\textsf{Bernoulli}(p\_i).$$
Translating this model to English, this model says: the player has a
...
Then $Q=\\frac{1}{M}\\sum\_{i=1}^M\\delta\_{p\_i}$, where
*δ*<sub>⋅</sub> is an [indicator
...
number of runs won
*k* ∣ *n*, *p* ∼ Binomial(*n*,*p*),
where $p=E\_Q\[p\_i\]=\\frac{1}{M}\\sum\_{i=1}^Mp\_i$ is the [expected
...
$$
x\_1\\mid p\_i\\sim \\textsf{Bernoulli}(p\_1),\\\\
x\_2\\mid x\_1, p\_2(x\_1) \\sim \\textsf{Bernoulli}(p\_2(x\_1)),\\\\
\\dots\\\\
x\_i\\mid x\_1, \\dots, x\_{i-1}, p\_i(x\_1, \\dots, x\_{i-1}) \\sim \\textsf{Bernoulli}(p\_i(x\_1, \\dots, x\_{i-1})),\\\\
\\dots\\\\
x\_n\\mid x\_1, \\dots, x\_{n-1}, p\_n(x\_1, \\dots, x\_{n-1}) \\sim \\textsf{Bernoulli}(p\_n(x\_1, \\dots, x\_{n-1})),\\\\
$$
...
![](spire_files/figure-markdown_strict/unnamed-chunk-14-1.png)
![](spire_files/figure-markdown_strict/unnamed-chunk-15-1.png)
...
highest levels as being somewhere in the *p* &gt; 0.9 range. While I
don’t doubt that this is true based on Merl’s recent performance, it’s
hard to currently say this with confidence from a statistical
perspective. For a sample of *n* = 50 runs, *one needs to win all 50
runs for a 95% confidence interval to rule out *p* &lt; 0.9*. For a
sample of *n* = 100 runs, *one needs to win all 96 runs for a 95%
confidence interval to rule out *p* &lt; 0.9*. We have not seen that
level of play on The Watcher in a large sample!
...
$\\varDelta=p\_1-p\_2$ denote the different between their true winrates.
...
<!-- # Streaks -->
<!-- \url{https://math.stackexchange.com/questions/383704/probability-of-streaks/383745#383745} -->
<!-- $$ S(n,k,p,q)= p^k\sum_{t=0}^\infty\binom{n-(t+1)k}{t}(-qp^k)^t - \sum_{t=0}^\infty\binom{n-tk}{t}(-qp^k)^t$$ -->
<!-- $$ S(n,k,p,q)= p^k\sum_{t=0}^{\lfloor\frac{n-k}{k+1}\rfloor}\binom{n-(t+1)k}{t}(-qp^k)^t - \sum_{t=0}^{\lfloor\frac{n}{k+1}\rfloor}\binom{n-tk}{t}(-qp^k)^t$$ -->
<!-- ```{r} -->
<!-- streak_prob <- function(n, k, p){ -->
<!--   q <- 1 - p -->
<!--   sum_1 <- 0 -->
<!--   sum_2 <- 0 -->
<!--   for(t in 0:floor((n - k) / (k + 1))){ -->
<!--     sum_1 <- sum_1 + choose(n - (t + 1) * k, t) * (-q * (p ^ k)) ^ t -->
<!--   } -->
<!--   if(floor(n / (k + 1)) >= 1){ -->
<!--     for(t in 1:floor(n / (k + 1))){ -->
<!--       sum_2 <- sum_2 + choose(n - t * k, t) * (-q * (p ^ k)) ^ t -->
<!--     } -->
<!--   } -->
<!--   print((p ^ k) * sum_1 - sum_2) -->
<!-- } -->
<!-- ``` -->
<!-- ```{r} -->
<!-- streak_prob(n = 50, k = 50, p = 0.95) -->
<!-- streak_prob(n = 100, k = 50, p = 0.95) -->
<!-- streak_prob(n = 200, k = 50, p = 0.95) -->
<!-- streak_prob(n = 50, k = 50, p = 0.9) -->
<!-- streak_prob(n = 100, k = 50, p = 0.9) -->
<!-- streak_prob(n = 200, k = 50, p = 0.9) -->
<!-- streak_prob(n = 50, k = 37, p = 0.95) -->
<!-- streak_prob(n = 100, k = 37, p = 0.95) -->
<!-- streak_prob(n = 200, k = 37, p = 0.95) -->
<!-- ``` -->
